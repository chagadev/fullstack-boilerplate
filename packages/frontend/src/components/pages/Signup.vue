<template>
  <div class="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8">
      ...
      <form class="mt-8 space-y-6" action="#" method="POST" @submit.prevent="signup()">
...
import { SignupDocument } from "../../generated/graphql-operations";
import Cookies from "js-cookie";

export default defineComponent({
  setup() {
    const { execute } = useMutation(SignupDocument);
    const email = ref();
    const password = ref();
    ...
    async function signup() {
      ...
      if (data.signup.token) {
        Cookies.set("JWT", data.signup.token);
        this.$router.push("/");
      } else {
        // TODO: Display Error
